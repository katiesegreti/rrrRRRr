...
# cl is factor of true classifications of the training set
...
sign_types <- signs$sign_type #answers to the training set
signs_pred <- knn(train = signs[-1], test = test_signs[-1], cl = sign_types)

# Create a confusion matrix of the predicted versus actual values
signs_actual <- test_signs$sign_type #answers to the test_signs
...
w1 <- locations %>%
  ...
  distinct()
weekday_evening <- w1 %>% filter(daytype == "weekday" & hourtype == "evening")
weekday_afternoon <- w1 %>% filter(daytype == "weekday" & hourtype == "afternoon")
weekend_afternoon <- w1 %>% filter(daytype == "weekend" & hourtype == "afternoon")
weekend_evening <- w1 %>% filter(daytype == "weekend" & hourtype == "evening")
...
predict(locmodel, weekend_evening, type = "prob")
#############*****************************************************
...
loan_model <- rpart(keep ~ loan_amount + credit_score, data = loans, method = "class", 
